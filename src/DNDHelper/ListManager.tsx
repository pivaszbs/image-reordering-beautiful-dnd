...
import {
    DragAndDropWrapper,
    Props
} from './DragAndDropWrapper/DragAndDropWrapper';
...
function ComponentWithMaxItems<T>(props: WithMaxItemsProps<T> & Omit<Props<T>, 'chunks'>) {
    ...
    const findChunkIndex = (id: string): number =>
        chunks.findIndex((chunk: Chunk<T>) => chunk.id === id);

    const onDragEnd: OnDragEndResponder = ({ source, destination }) => {
        if (destination) {
            const { index: indexInSourceChunk, droppableId: sourceChunkId } =
                source;
            const {
            ...
            const destinationChunkIndex: number =
                findChunkIndex(destinationChunkId);
